...
if deployment_settings.has_module("project"):
    ...
    def project_tables():
        ...
        table = db.define_table(tablename,
                                ...
                                donor_id(readable=False,
                                         writable=False),
                                sector_id(readable=False,
                                          writable=False),
                                need_type_id(readable=False,
                                             writable=False),
                                #subsector_id(),
                                #Field("quantity"),
                                ...
                                Field("total_hours", "double",
                                      label = T("No. of Hours")),
                                Field("total_vols", "integer",
                                      label = T("No. of Volunteers")),
        ...
        # ---------------------------------------------------------------------
        def project_activity_controller():
            ...
            if not s3_has_role(STAFF):
                # Only show this Org's Activities
                response.s3.filter = (table.organisation_id == auth.user.organisation_id)
                table.organisation_id.default = auth.user.organisation_id
                ...
                if record:
                    ...
                    rheader = DIV( TABLE(
                                       ...
                                       TR( TH( "%s: " % T("Organization")),
                                           ...
                                           #TH( "%s: " % SECTOR),
                                           #shn_org_sector_represent(record.sector_id),
                                         ),
                                        ),
                                    rheader_tabs
                                    )
        ...
        return dict(
            ...
            project_activity_controller = project_activity_controller
            )
