                            ...
                            Field("lead_time",
                                  ...
                                  comment = DIV(_class="tooltip",
                                                _title="%s|%s" % (T("Lead Time"),
                                                                  T("Time until Goods can be available for donation.")
                                                                  ),
                                                ),
                                  writable = False,
                                  readable = False,
...
S3FilterFieldChange({
 ...
 'fncRepresent':fncRepresentItem
            ...
            if recv_record:
                rheader = DIV( TABLE(
                                   ...
                                   TR( TH( "%s: " % T("Sent By Person")),
                                       vita.fullname(recv_record.sender_id),
                ...
                if recv_record.status == SHIP_STATUS_IN_PROCESS:
                    if auth.s3_has_permission("update",
                        ...
                        s3.rfooter.append(recv_btn)
                        s3.rfooter.append(recv_btn_confirm)
                else:
                    ...
                    if recv_record.status != SHIP_STATUS_CANCEL:
                        if auth.s3_has_permission("delete",
                            ...
                            s3.rfooter.append(cancel_btn)
                            s3.rfooter.append(cancel_btn_confirm)
        ...
        def prep(r):
            ...
            if r.record:
                if r.record.status == SHIP_STATUS_IN_PROCESS:
                    crud_strings.inv_recv.title_update = \
                    crud_strings.inv_recv.title_display = T("Process Received Shipment")
            return True
    ...
    def inv_recv_form (r, **attr):
        ...
        table.site_id.represent = shn_site_represent
    ...
    def inv_incoming():
        ...
        if has_module("cr"):
            rheader_dict["cr_shelter"] = s3.shelter_rheader
        if has_module("hms"):
            rheader_dict["hms_hospital"] = hms_hospital_rheader

        s3.filter = (db.inv_send.status == SHIP_STATUS_SENT) & \
                             (db.inv_send.to_location_id == to_location_id)

        configure("inv_send", insertable=False)

        s3.actions = [dict(url = URL(c="inv", f="send",
                                              args = ["[id]", "send_item"],
            ...
            if send_record:
                ...
                if send_record.status == SHIP_STATUS_IN_PROCESS:
                    if auth.s3_has_permission("update",
                        ...
                        s3.rfooter.append(send_btn)
                        s3.rfooter.append(send_btn_confirm)
                else:
                        ...
                        if send_record.status == SHIP_STATUS_SENT:
                            ...
                            if auth.s3_has_permission("update",
                                ...
                                if "received" in request.vars:
                                    ...
                                    s3.rfooter.append(receive_btn)
                                    s3.rfooter.append(receive_btn_confirm)
                            if auth.s3_has_permission("delete",
                                                      db.inv_send,
                                                      record_id=send_record.id):
                                cancel_btn = A( T("Cancel Shipment"),
    ...
    def inv_send_controller():
        ...
        def prep(r):
            ...
            if r.record:
                ...
                if r.record.status == SHIP_STATUS_IN_PROCESS:
                    crud_strings.inv_send.title_update = \
                    crud_strings.inv_send.title_display = T("Process Shipment to Send")
                elif "site_id" in request.vars and r.record.status == SHIP_STATUS_SENT:
                    crud_strings.inv_send.title_update = \
                    crud_strings.inv_send.title_display = T("Review Incoming Shipment to Receive")
