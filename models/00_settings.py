...
if request.controller == "vol" and request.function == "req":
    # Redirect to Registration Form, not Login Form
    _next = URL(args=request.args, vars=request.vars)
    auth.permission.loginpage = URL(c="vol", f="register",
                                    vars=dict(_next=_next))

auth.messages.login_disabled = "Please call the Emergency Management Department at (213) 484-4800 about your Volunteer Status."

auth.permission.AUTHENTICATION_REQUIRED = T("Registration Required")
...
auth.messages.verify_email = T("Click on the link %(base_url)s/default/user/verify_email/%(key)s to verify your email.") % \
                             dict(base_url = s3.base_url,
                                  key = "%(key)s")
...
auth.messages.reset_password = T("Click on the link %(base_url)s/default/user/reset_password/%(key)s to reset your password.") % \
                             dict(base_url = s3.base_url,
                                  key = "%(key)s")
...
_table_user.language.default = deployment_settings.get_L10n_default_language()

# Languages available in User Profiles
if len(s3.l10n_languages) > 1:
    _table_user.language.requires = IS_IN_SET(s3.l10n_languages,
                                              zero=None)
else:
    _table_user.language.default = s3.l10n_languages.keys()[0]
...
_table_user.language.represent = lambda opt: s3.l10n_languages.get(opt, UNKNOWN_OPT)

# For the Registration & User Profile:
...
if s3_has_role(STAFF):
    # 24h for Staff
    deployment_settings.L10n.datetime_format = T("%m-%d-%Y %H:%M")
    deployment_settings.L10n.time_format = T("%H:%M")
else:
    auth.messages.logged_out = "Signed Out - Thank you for supporting the City of Los Angeles"
######
# CRUD
######
...
def s3_formstyle(id, label, widget, comment, hidden=False):
    ...
    if hidden:
        attr = dict(_class = "hidden")
    else:
        attr = {}

    # Label on the 1st row
    row.append(TR(TD(label, _class="w2p_fl"), TD(""), _id=id + "1", **attr))
    # Widget & Comment on the 2nd Row
    row.append(TR(widget, TD(comment, _class="w2p_fc"), _id=id, **attr))
...
s3.messages.confirmation_email_subject = T("%(system_name)s access granted") % \
    dict(system_name=system_name)
s3.messages.confirmation_email = T("Welcome to the %(system_name)s Portal at %(base_url)s. Thanks for your assistance.") % \
    dict(system_name=system_name,
         base_url=s3.base_url)
