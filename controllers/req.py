...
# -----------------------------------------------------------------------------
def req():
    """ REST Controller for non-interactive ONLY"""

    if "rcomponents" not in request.get_vars:
        request.get_vars.update(rcomponents="pr_contact")

    response.s3.prep = lambda r: not r.interactive
            ...
            if record.type == 1:
                ...
                rheader = DIV( TABLE( TR( TH( "%s: " % T("Request")),
                                      ...
                                      TR( TH( "%s: " % T("Committing Inventory")),
                                          shn_site_represent(record.site_id),
                                          TH( "%s: " % T("Commit Date")),
