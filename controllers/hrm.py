...
shn_menu(module, prep=shn_menu_prep)
...
def hrm_map_popup(r):
    ...
    output.append(TR(TD(B("%s:" % T("Name"))),
                     TD(vita.fullname(r.record.person_id))))
...
def compose():
    ...
    return s3.msg_compose(redirect_module = module,
                          redirect_function = "compose",
                          redirect_vars = {fieldname: id},
                          title_name = title)

# =============================================================================
# Adding a human resource inline
# =============================================================================
...
def create_inline():
    ...
    # Set up fields
    pr_person = db.pr_person
    hrm_human_resource = db.hrm_human_resource
    ...
    fields = [
        ...
        hrm_human_resource.job_title,
    ]

    emailRequired = (
        request.controller == "hrm" and
        deployment_settings.get_hrm_email_required()
    )

    if emailRequired:
        email_validator = IS_EMAIL()
    else:
        email_validator = IS_NULL_OR(IS_EMAIL())

    email_field_name = "email_address"
    mobile_phone_field_name = "mobile_phone"
    fields.extend([
        Field(
            "organisation_id",
            notnull=True,
            requires = hrm_human_resource.organisation_id.requires,
            label = hrm_human_resource.organisation_id.label,
            represent = hrm_human_resource.organisation_id.represent,
        ),
        Field(
            email_field_name,
            notnull=emailRequired,
            requires=email_validator,
            label=T("Email Address")
        ),
        Field(
            mobile_phone_field_name,
    ...
    form = SQLFORM.factory(table_name="hrm_human_resource",
                           labels=labels,
                           formstyle=formstyle,
                           separator = "",
                           *fields)
    ...
    existing_mobile_phone_contact = None
    existing_person = None
    existing_human_resource = None

    # look for a person with the same email address
    email_address = _vars[email_field_name]
    if email_address:
        existing_email_contact = existing(
            (pr_contact.contact_method == "EMAIL") &
            (pr_contact.value == email_address)
        )
        if existing_email_contact is not None:
            # check every other contact detail matches, otherwise complain

            # mobile phone
            existing_mobile_phone_contact = existing(
                (pr_contact.contact_method == "MOBILE PHONE") &
                (pr_contact.pe_id == existing_email_contact.pe_id)
            )
            if existing_mobile_phone_contact is not None:
                check_request_var_matches_value_if_given(
                    "mobile_phone",
                    existing_mobile_phone_contact.value
                )

            # person details
            existing_person = existing(
                ptable.pe_id == existing_email_contact.pe_id
            )
            if existing_person is not None:
                check_request_var_matches_value_if_given(
                    "first_name",
                    existing_person.first_name
                )
                check_request_var_matches_value_if_given(
                    "middle_name",
                    existing_person.middle_name
                )
                check_request_var_matches_value_if_given(
                    "last_name",
                    existing_person.last_name
                )

                # human resource
                existing_human_resource = existing(
                    hrm_human_resource.person_id == existing_person.id
                )
                if existing_human_resource is not None:
                    check_request_var_matches_value_if_given(
                        "job_title",
                        existing_human_resource.job_title
                    )
                    check_request_var_matches_value_if_given(
                        "organisation_id",
                        existing_human_resource.organisation_id
                    )

    if accepted: # create objects as necessary
        # an email address is assumed to uniquely identify a person
        # however, there is no guarantee that a person has an email address
        if existing_person is None:
            person_id = ptable.insert(first_name = _vars["first_name"],
                                      middle_name = _vars["middle_name"],
                                      last_name = _vars["last_name"])
            s3mgr.model.update_super(ptable, {"id":person_id})
            query = ptable.id == person_id
            person = db(query).select(ptable.pe_id,
                                      limitby=(0, 1)).first()
            pe_id = person.pe_id
        else:
            pe_id = existing_person.pe_id
            person_id = existing_person.id

        if existing_email_contact is None:
            db.pr_contact.insert(pe_id = pe_id,
                                 contact_method = "EMAIL",
                                 value = email_address)

        if existing_mobile_phone_contact is None:
            mobile_phone = _vars[mobile_phone_field_name]
            if mobile_phone:
                db.pr_contact.insert(pe_id = pe_id,
                                     contact_method = "MOBILE PHONE",
                                     value = mobile_phone)

        if form.errors:
            response.flash = "form is invalid"
        else:
            if existing_human_resource:
                result.update(
                    created_object_id = existing_human_resource.id,
                    created_object_representation = hrm_human_resource_represent(existing_human_resource)
                )
                # nothing needs adding?
            else:
                created_human_resource = db.hrm_human_resource.insert(
                    person_id = person_id,
                    job_title = _vars["job_title"],
                    organisation_id = _vars["organisation_id"]
                )
                db.commit()
                result.update(
                    created_object_id = created_human_resource.id,
                    created_object_representation = hrm_human_resource_represent(created_human_resource)
                )
