...
def register_onaccept(form):
    """
        LA-specific:
            Ensure manually-created users get added to 'Staff' role &
            have an HRM record created.
    """
    ...
    # Add to 'Staff' role
    table = db.auth_group
    STAFF = db(table.uuid == "STAFF").select(table.id,
                                             limitby=(0, 1)).first().id
    table = db.pr_person
    person_uuid = db(table.id == person).select(table.uuid,
                                                limitby=(0, 1)).first().uuid
    table = db.auth_user
    query = (table.person_uuid == person_uuid)
    user = db(query).select(table.id,
                            limitby=(0, 1)).first().id
    table = db.auth_membership
    table.insert(user_id = user,
                 group_id = STAFF)

    # Create an HRM record so that the user appears in human_resource_id() lookups
...
def user():
    ...
    s3mgr.configure(tablename,
                    ...
                    create_onaccept = register_onaccept)

    # Staff don't need a Mobile Phone
    ...
    site.requires = IS_NULL_OR(IS_ONE_OF(db, "org_site.id",
                                         ...
                                         sort=True))
    site.represent = shn_site_represent
...
def rostermail():
    """
        Email Addresses for the Volunteer Roster to be sent to
    """
